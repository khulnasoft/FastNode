...
from fastnode.ui import schema_utils, streamlit_utils
from fastnode.ui.streamlit_utils import CUSTOM_STREAMLIT_CSS, SessionState

STREAMLIT_RUNNER_SNIPPET = """
from fastnode.ui import render_streamlit_ui
from fastnode import Fastnode

import streamlit as st

st.set_page_config(page_title="Fastnode", page_icon=":arrow_forward:")

with st.spinner("Loading Fastnode. Please wait..."):
    fastnode = Fastnode("{fastnode_path}")

render_streamlit_ui(fastnode)
"""


def launch_ui(fastnode_path: str, port: int = 8501) -> None:
    ...
    with NamedTemporaryFile(
        ...
        f.write(STREAMLIT_RUNNER_SNIPPET.format(fastnode_path=fastnode_path))
        f.seek(0)

        # TODO: PYTHONPATH="$PYTHONPATH:/workspace/fastnode/src"
        import subprocess
...
def render_streamlit_ui(fastnode: Fastnode) -> None:
    ...
    if fastnode.description:
        st.markdown(fastnode.description)

    InputUI(session_state=session_state, input_class=fastnode.input_type).render_ui()
        ...
        with st.spinner("Executing operation. Please wait..."):
            try:
                input_data_obj = parse_obj_as(
                    fastnode.input_type, session_state.input_data
                )
                session_state.output_data = fastnode(input=input_data_obj)
