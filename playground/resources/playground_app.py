...
st.set_page_config(page_title="Fastnode Playground", page_icon=":arrow_forward:")

st.title("Fastnode Playground")
...
BADGES = """
<a href="https://gitHub.com/khulnasoft/fastnode" title="Star Repo"><img src="https://img.shields.io/github/stars/khulnasoft/fastnode.svg?logo=github&style=social"></a>
...
st.markdown(
    "Fastnode enables you to instantly to turn your python functions into microservices with auto-generated HTTP API, interactive UI, and more."
...
with col1:
    ...
    # st.code("fastnode launch-ui app:seperate_audio", language="plain")
with col2:
    open_api = st.button("🔌 Launch API")
    # st.code("fastnode launch-api app:seperate_audio", language="plain")
...
with st.beta_expander("Export this Fastnode"):
    st.markdown(
        "Fastnode provides capabilities to seamlessly export your services into portable, shareable, and executable files or Docker images."
    )
    col1, col2 = st.beta_columns([1, 2])
    with col1:
    ...
    with col2:
        st.code(
            f"fastnode export app:{selected_demo} my-fastnode.zip", language="plain"
        )
    ...
    with col2:
        st.code(
            f"fastnode export app:{selected_demo} --format docker my-fastnode:latest",
            language="plain",
        )
...
if open_docker_export_feature:
    open_link("https://github.com/khulnasoft/fastnode/issues/4")
    open_docker_export_feature = False

if open_zip_export_feature:
    open_link("https://github.com/khulnasoft/fastnode/issues/3")
